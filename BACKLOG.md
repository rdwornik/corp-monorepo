...
**Themes (backbone):** [E1] Knowledge retrieval & analytics · [E2] Ingest reliability · [E3] File lifecycle & inbox automation · [E4] Model evaluation & local tiers · [E5] Safety & code health

---

## [E1] Knowledge retrieval & analytics
> As an analyst, I want every benchmark query answerable across all knowledge sources, so retrieval is complete and unified.

### [S1] Close the analytics benchmark and unify RFP + vault retrieval
...
## [E3] File lifecycle & inbox automation
...
### [S3] Bring MyWork naming and Outlook content into the pipeline
...
## [E4] Model evaluation & local tiers
...
### [S4] Run the skill-eval checkpoint and explore a local extraction tier
...
### [S5] Harden the cleanup pipeline and clear verified dead code
...
- [#13] [P3][M] Sanitize fixtures and archives of real internal paths — `scripts/phase2_plan.json` (full OneDrive path + "Nestle Slides" filename), `tests/extractor/fixtures/training_data_raw.json` + `classifier_training*.json` (source_path with `C:/Users/1028120/Documents/MyWork/...` + Nestle Purina recording title), `models/classifier_train.json` (same real source path), `docs/archive/2026-03-26_VAULT_MANIFEST.json` (Nestle Purina filename + client field) — replace real OneDrive/MyWork paths and internal recording titles with synthetic equivalents · Done when: pre-push sensitivity sweep rerun shows zero real internal paths in tracked files · refs sensitivity sweep 2026-06-06
...
**Grooming log:** 2026-05-18 (stream-format seed from retired `docs/HANDOFF.md`) · 2026-06-02 (story-map migration; 9 open items preserved, 3 closed items retired) · 2026-06-03 (added "Safety & code health" theme; preserved #10 path-traversal security, #11 dead code from stale branches; added #12 ruff drift) · 2026-06-06 (added #13 sanitize-fixtures-archives from sensitivity sweep) · 2026-07-07 (added methodology-adoption story + #14, hub #221 n=2 deploy) · 2026-07-07 (closed #14 — methodology corpus v1.2.0 adopted, all three Done-when clauses met; final clause — hub registry `deployed_methodology_version: 1.2.0` n=2 — landed at hub `22def9f`; single-task "Adopt the ecosystem methodology corpus" story retired per ADR-65, full text in git history) · 2026-07-13 (T1 canonical baseline: added `[E1]`–`[E5]` theme ids + `[S1]`–`[S5]` story ids per content-parity inventory D1; task `[#id]`s unchanged; wired the `validate-backlog` plugin twin as an early-adoption gate, ADR-78) · 2026-07-16 (added+closed #15 — hardcoded video mime in transcript generation; ticket origin `daa8578` on the retired `docs/backlog-transcript-mime-fix` branch, the sole prior home of the #15 text — never landed on `main`; fix threaded `uploaded.mime_type` through `extract.py`→`ExtractionResult`→`generate_transcript`, closing merge `af95fa7`; both Done-when clauses satisfied; retired per ADR-65, superseded branch deleted). Next quarterly: 2026-07-01.
