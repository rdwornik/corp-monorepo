...
- [#19] [P2][S] Arc-B batch 1 — KILL `cost_tracker` (whole file + 2 of 23 methods in `test_providers.py`) · Done when: `cost_tracker.py` is removed, its 2 tests dropped, suite green · refs signed manifest Batch 1 · R4
- [#20] [P2][M] Arc-B batch 2 — repoint the facts pipeline: kill facts DDL/loader in `index_builder.py`, point `query_engine.search_facts`→`_search_notes_fts`, then `corp index rebuild` · Done when: 3 live consumers read via `notes_fts`, facts residue cleared by rebuild, suite green · refs signed manifest Batch 2 · R4
...
- [#29] [P2][L] Migrate `config` → the `PipelineConfig` family (migrate all `corp.config` importers) — the capstone unification · Done when: production config resolves through `PipelineConfig` and `corp.config` importers are migrated, tach clean · depends-on: #19, #20, #21, #22, #23 · refs R5 (cut before unify)
