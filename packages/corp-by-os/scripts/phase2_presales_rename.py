...
def print_plan_summary(plan: list[dict], dest_base: Path) -> None:
    ...
    legend = []
    if ambig:
        legend.append("! = ambiguous date")
    if no_date:
        legend.append("* = date from mtime")
    if unknown:
        legend.append("_Unknown = client not identified")
    if legend:
        print("\n  " + "  |  ".join(legend))
...
def diff_plans(old_path: Path, new_path: Path) -> None:
    ...
    if diff_rows:
        ...
        for fname, oc, nc in diff_rows[:50]:
            marker = (
                "+"
                if nc != "_Unknown" and oc == "_Unknown"
                else ("-" if oc != "_Unknown" and nc == "_Unknown" else " ")
            )
