...
| test_api_keys.py | 3 | API key availability |
...
| test_router_acceptance.py | 14 | Retry logic, model registry, vault context |
| test_validate_profiles.py | 32 | Profile contradictions, auto-fix |
| test_vault_adapter.py | 11 | Vault retrieval, SQLite fallback, --rfp-only |
...
- 246 bare `print()` calls across 9 src files (see docs/STRUCTURAL_DEBT.md)
- 7 argparse CLIs need Click migration (see docs/STRUCTURAL_DEBT.md)
- 1 `sys.path.insert` hack in `rfp_answer_word.py` — fix during namespace migration
