...
### 2026-07-05 — Deep Audit F: MyWork estate & naming (Wave 2)
- Did: Read-only estate audit on `docs/deep-mywork`: metadata scan of local MyWork (2,236 files / ~10.2 GB across 7 zones + .corp + 15_Extra_Inititives); zone census with purpose-vs-reality verdicts; ADR-14 conformance measured with the authoritative naming_config.yaml regex (22 types / 32 aliases); 15_Extra disposition evidence (2 permitted sample-reads); template inventory; .corp vs repo config split; addressing-kernel inputs + 11 backlog seeds. OneDrive never traversed (mirror figures cited from 2026-06-16 audit).
- Result: ADR-14 conformance is 10/2,236 files (0.4%) — all 2026-03 MISC_GEN fallbacks; zero organic adoption. Six competing conventions measured. 196 byte-identical space/underscore twin pairs (~756 MB) in 10_Projects. 20_Workflows cold while the Warsaw folder in 30_Reference is the real production studio (96% of the zone, active 07-05). 90_Archive empty despite JOURNAL 03-29 claiming 9 projects moved there (discrepancy for operator). content_registry.yaml split-brained (repo 4.8 KB vs .corp 21.4 KB); routing_map/config.json/solution_matrix orphans with no repo twin. No template_registry.yaml on disk; template_manager.py points at two phantom zones. Plaintext Databricks PAT in 15_Extra flagged (values not reproduced).
- Changes: `docs/audits/2026-07-05-deep-mywork.md` (new), `docs/audits/2026-07-05-deep-mywork.html` (new), this entry.
- Abandoned: Project_Codes.xlsm column contract (binary, Audit E scope); dup-pair hash verification beyond size-equality (196/197 equal-size sufficed for evidence).
- Next: operator decisions on seeds F-3/F-4/F-5/F-6/F-8/F-10/F-11; branch left unmerged for Wave-2 review.

### 2026-07-05 — Deep Audit D: vault + metadata chain (Wave 2, read-only)
- Did: Traced the metadata chain field-by-field (CKE emit → schema → vault → index.db → retrieval → answer context), ran a full vault census + link-graph baseline from files (Obsidian never launched; CLI never executed — presence verified by file/version metadata only), quantified all hygiene classes, scoped W5 canonicalization, and graded the proposed Obsidian Knowledge Operating Model element-by-element. Branch `docs/deep-vault` off `2753601`. All SQLite `mode=ro`; zero vault writes; zero OneDrive reads/traversal; census computed by scratchpad scripts importing `corp.schema.validate` in-memory.
- Result: 2 deliverables in `docs/audits/`. Headlines — only 6 of 30+ frontmatter fields are load-bearing (title/topics/products/client/source_type/trust_level); the extract template renames `source_file`→`source` so **all 813 knowledge notes fail the written S1 contract (0%)**, recoverable to 87% with two mechanical fixes, ~99% with a full enum map; ingest bypasses `validate_frontmatter` entirely and drops the client it reads from the staging path (origin of much of the 258-clientless population, of which 202 are deterministically recoverable and 141 are legitimately client-free library notes); the 850-vs-488 delta closes exactly (325 dedup losers + 22 quarantine + 12 navigation + 1 parse-fail); link tree is 0% built — 100% orphans, 0 MOC-reachable (all 9 MOCs are dataview), 8,262 dangling links whose top-30 targets cover 78% and already name the future spine; **effective RFP corpus is 39 notes, not 182** (143 of the rfp_visible rows are deprecated, saved only by the retrieve-time filter); 326 de-facto S3 tombstones with 0 sources resolving at recorded paths (63 re-findable by basename in MyWork); products field: 1,975 instances / 322 terms, 62% resolve today, real mapping need ≈80 aliases + drop list (~270 instances are non-BY tech leakage); `resolve_product_key` has zero call sites — evidence favors enforcing canonicalization at index build. Operating-model grades: elem 1 AMEND (generate spine from dangling-link taxonomy; dataview invisible to link audits), elem 2 AMEND (contract reconciliation prerequisite; 48h SLA needs a scheduled mechanism), elem 3 RATIFY-with-dependency, elem 4 RATIFY/EXISTS-UNTESTED (Obsidian 1.12.4 + `Obsidian.com` CLI shim present on PATH), elem 5 AMEND (S3-triage + dedup disposition before backfill; real backfill universe ≈487 notes). Must-adds: index.db as declared derived cache; S3 ⇒ rfp_visible=0.
- Changes: `docs/audits/2026-07-05-deep-vault-metadata.md` (`47f0882`), `docs/audits/2026-07-05-deep-vault-metadata.html` (`39aa690`), this JOURNAL entry. Branch NOT merged — feeds F0 hygiene, W5, and operating-model ratification per mission.
- Abandoned: nothing. (No hygiene fixes performed — quantify-only mandate; 325 dedup-loser files and 326 tombstones reported, untouched.)
- Next: Operator decisions queued in the backlog-seed table (V1 rename direction, V2 archive-vs-delete, V3 dedup deletion approval, W5 seam ruling); OM1 spine generation is the cheapest structural win (top-30 hubs = 78% of dangling links); join OM3 (key_facts/overlays discarded at answer layer) with Audit C's emit-side findings.

### 2026-07-05 — Deep Audit C: knowledge extraction (Wave 2, read-only)
- Did: Settled the 2026-06-21 extractor-brainstorm verify-first triad by reading code, established the emit-side output→vault contract, and produced R2 gap-fill backlog seeds, on branch `docs/deep-extraction` (worktree, off `2753601`). Evidence: code trace with file:line on every claim; sampled 203 real v3 notes + 203 JSON sidecars (5,046 facts) read-only from the main checkout's gitignored `data/_outputs/v3`; `overnight_state.db` queried `mode=ro`. No extraction runs, no LLM calls, no OneDrive traversal.
- Result: Triad verdicts — sampler EXISTS (FFmpeg scene detection live on the `cke process` lane, which produced the whole v3 corpus; manifest lane still uniform; histogram dedup, not pHash); transcripts GENERATED (dedicated 2nd Gemini call; no diarization; ingest skips `*_transcript.md` so they never reach the vault); per-fact span grounding GAP (number cross-ref only; non-numeric facts auto-"verified" → the 93.6% verified rate is inflated; `locator` is an unverified LLM page ref with zero consumers). Contract headlines — the JSON sidecar (all enriched facts) is skipped by ingest; v2.1 provenance fields are validated into the manifest then silently dropped by CKE's parser while the index reserves 4 columns for them; Lane B (`move_to_vault`) never landed a vault write and would create a literal `02_sources/{project_id}` folder. Telemetry autopsies — 601 files `tier='pending'` is a hardcoded literal with no update path; per-file cost is an average of an estimate; `cost_log.jsonl` ledger and ops `extractions` writers have ~0 reachable call sites; 4 stuck `running` runs = no try/finally + no janitor (crash-retry signature in run timestamps); Batch API = 649 tested lines worth ~$0.12/corpus-run, never used, corp `batches` table writer-less; facts pipeline has 3 independent breaks (kill-or-fix consequence lists provided, operator leans kill). Corrected Phase-3: `routing_map.yaml` HAS registered runtime readers (`corp extract`, `corp overnight`); its *targets* are the phantom part. 11 backlog seeds (E1–E11); E5 (facts) + E7 (Lane B) are the blocking rulings.
- Changes: `docs/audits/2026-07-05-deep-extraction.md` (new, `b29b2a5`), `docs/audits/2026-07-05-deep-extraction.html` (new, self-contained process map, `51cccbf`), this JOURNAL entry. Branch NOT merged — handed back per Wave-2 protocol.
- Abandoned: nothing. (Dead code found — `frames/extractor.py`, Batch API limb, orphaned `batches` table — reported, not deleted; read-only mandate.)
- Next: Architect rulings on E5 (facts kill/fix) and E7 (Lane B repair/retire, joint with Audit D's vault-zone verdict); decision-free hygiene E3 (overnight terminal states) + E10 (cost truth) can start anytime; Audit D joins the emit field table on the [H]-marked handshake fields.

### 2026-07-05 — Deep Audit B: magistrala/capture (Wave 2, read-only)

- Did: Deepened Phase-1 W1/W2 findings on branch `docs/deep-magistrala` (worktree). Function-level trace of both ingest lanes + parity table; config-truth pass over routing_map/content_registry/naming_config/.corp-vs-repo copies with disk-existence sweep of every routing destination; metadata-only triage of the 75-item inbox backlog with dry replication of registry matching (no pipeline execution, no moves); dedup/identity zero-rows forensics via git dates + ops.db mode=ro.
- Result: Verdict — machinery DORMANT-but-sound, its map BROKEN. Key evidence: live registry (2026-03-23) predates Council #24 and only 6/42 destinations exist (both lanes mkdir → structure-drift risk); Phase-1 finding 10 amended — routing_map.yaml IS consumed by `corp extract`/`corp overnight` and would mkdir literal `02_sources/{project_id}` in the vault; `datasketch` never installed → MinHash import-dead/silent; all identity/learning surfaces (routing_feedback 03-25, extractions 03-25, content_signatures 03-27) wired AFTER last real lane use (03-24) — zero rows explained; registry priority declared (rules>clients) contradicts coded order (clients>rules); ~55% of registry file has zero code consumers; interactive lane sees 0 backlog files (all in folders), batch would quarantine both folders whole; per-file dry match: 28/72 matched, 61% quarantine; backlog = curated 41-file RFP corpus (R1 seed, incl. Rolls-Royce APS) + 31-file training kit (W1 pilot candidate; Credentials.xlsx must never be extracted); ADR-14 drift confirmed (19/15 body vs 22/32 live; example codes DECK/CERT/QA/PROP exist in no generation); `corp doctor` validates the stale repo registry copy, not the live one.
- Changes: `docs/audits/2026-07-05-deep-magistrala.md` (6e780a3), `docs/audits/2026-07-05-deep-magistrala.html` (82e054a), this entry.
- Abandoned: nothing; sandbox re-execution deliberately skipped (Phase 1 settled 5/5).
- Next: Operator decisions per backlog-seed table (registry v4 scope, routing_map rewrite-vs-guard, lane choice, RFP-corpus custody, BRC/LabelVie active-deal check); then F0-1..4 remediation; branch left unmerged per Wave-2 rules.

### 2026-07-05 — Functional process audit (read-only mega-mission, docs-only)
- Did: Reverse-engineered the business processes the repo actually implements and measured which are alive, on branch `docs/2026-07-05-functional-audit` (off `fb9b2dd` after ff-pull). Triangulated capability (code trace via 3 parallel read-only subagents) × intent (docs/ADRs) × use (DB telemetry `mode=ro`, file mtimes, git archaeology, JOURNAL). One gated execution only: `corp test-pipeline` sandbox smoke (isolation to `%TEMP%` proven pre-run, cleanup verified) — 5/5 PASS, settling the "magistrala unverified since Council #24" question for the tested legs. No OneDrive path touched; no code/config/data changed.
- Result: 5 audit deliverables in `docs/audits/`. Headlines — all 3 DBs frozen since 2026-03-28 and every learning surface (`routing_feedback`, `registry_suggestions`, `content_signatures`, `facts`, `extractions`) has zero rows ever; the RFP process has answered zero real RFPs and exists as two disjoint stacks with the 1,329-file RFP KB unreachable by any code path (ADR-22 ratified-not-built); knowledge loop DORMANT-not-broken (sandbox-alive, inbox inflow stopped 2026-04-10, 75-file backlog); `com`/deal-loop EXISTS-UNTESTED with `PROJECTS_ROOT` mispointed at `./test_projects` and SF/SharePoint legs MISSING; X1 backup confirmed MISSING (vault git has no remote; <100 MB of irreplaceable data unbacked vs 18.4 GB regenerable `_outputs`). Full suite green post-audit: 2564 passed / 6 skipped; tree clean.
- Changes: `docs/audits/2026-07-05-functional-{telemetry,scenarios,process-inventory,artifact-lifecycle,synthesis}.md` (new; commits `c03b53d`, `d0f16ba`, `b8f9bf1`, `e3aae05`, `2edff21`), this JOURNAL entry. Branch NOT merged — handed back for architect review per mission.
- Abandoned: nothing. (Leaked March staging dirs and 18.4 GB disposable `_outputs` reported, not deleted — read-only mandate.)
- Next: Architect reviews branch → operator triage of the 7 open questions in the synthesis (RFP KB provenance, style-corpus seed, com fix-vs-kill, sync-mywork status, W1 lane choice, inbox backlog, 15_Extra_Inititives); F0 quick wins flagged: vault git remote + backup, index rebuild, config-truth pass.

### 2026-06-16 — Current-state architecture audit (read-only ecosystem measurement tool)
- Did: Built a permanent, read-only ecosystem audit tool (`scripts/current_state_audit.py` + `scripts/_audit_core.py`, Tach-excluded) that inventories four roots — corp-monorepo, MyWork, ObsidianVault, and (operator-authorized, opt-in `--include-onedrive`) the synced mirror — and emits a deterministic inventory JSON; then authored an L0–L5 gap report classifying the ecosystem against the target "night-shift pre-sales brain" ideal. Eight revertable commits (scaffold+AST guard → filesystem inventory → naming entropy → duplication → source-of-truth → automation inventory → run+report → review fixes). Read-only proven in code: a single guarded write sink, an AST invariant test (no mutation/shell-out except the whitelisted `write_text`), and a runtime `WriteLedger` assertion; cloud-only placeholders detected via `st_file_attributes` and never hydrated (hash only local files ≤50 MB). Mirror hashing is a separate `--hash-onedrive` opt-in (default metadata-only). Reused `corp.cleanup._guard_onedrive` by import (anti-drift). Ran `/codex-review` (4 HIGH, 0 critical) and fixed all four (no-hydration/bounded `_read_text_safe`, symlink-safe automation probes via `_is_plain_dir`, populated `hashed_count`, scandir errors surfaced into `PathInventory.errors`).
- Result: Two durable deliverables in `docs/audits/`: the inventory JSON (1.3 MB; `ledger == [json]`, no hydration) and the gap report (six layers classified EXISTS/PARTIAL/MISSING with file-path evidence + 5-line exec summary). Headlines — synced mirror 159,826 files / 1.1 TB (96% cloud-only); the declared immutable `02_sources` zone has no physical folder on disk; 44–83% of files match no naming convention; 2.5 GB local exact-duplicate waste; the deterministic spine (L2) is the only EXISTS layer. 33 audit safety tests pass; full suite 2581 passed / 7 skipped / 1 pre-existing env-only failure (`ANTHROPIC_API_KEY` unset, unrelated to this work). ruff + tach clean. No new folders; no scanned-path mutations.
- Changes: `scripts/current_state_audit.py` (new), `scripts/_audit_core.py` (new), `config/audit.yaml` (new), `tests/safety/test_audit_readonly_invariant.py` (new), `tests/safety/test_audit_core.py` (new), `docs/audits/2026-06-16-current-state-architecture-audit-inventory.json` (new), `docs/audits/2026-06-16-current-state-architecture-audit.md` (new), `docs/audits/2026-06-16-codex-current-state-audit-tool.md` (codex review artifact), this JOURNAL entry. Branch `audit/current-state-architecture`, merged `--no-ff`.
- Abandoned: nothing. (Mirror hash-level overlap deferred to `--hash-onedrive` / `corp.cleanup.disk.find_onedrive_overlap`; restructuring/taxonomy deferred to the next AI Council phase by design — this pass measures only.)
- Next: AI Council phase chooses the target taxonomy from this baseline; re-run the tool to measure gap closure (cached JSON enables re-classification without re-walking the mirror). Consider enumerating the audit tool as a read-source in `~/.claude/rules/core-invariants.md` (a permanent script now reads the mirror under explicit authorization).

### 2026-06-06 — Triage ratification: catch-up journal for 2026-06-02 Tier-1 lifecycle plugin install
- Did: Ratified the single surviving med-severity finding from the 2026-06-06 nightly conformance digest (`docs/audits/2026-06-06-conformance-nightly-digest.md`, marker `<!-- counts: raw=2 survived=1 killed=1 -->`, issue #4) — the 2026-06-02 Tier-1 lifecycle plugin install (three substantive commits) was never journaled, violating CLAUDE.md §6 "Changes: is the sole change record". This is a TODAY-dated catch-up entry (prepended, NOT backdated mid-history) recording that prior work: `13fea1e` chore: install Tier-1 lifecycle plugin config (ADR-70 #73 Unit-5b) — `.claude/settings.json` +14 (plugin config block); `a7a161e` chore: update settings.json to CLI-canonical key order after plugin install — `.claude/settings.json` key reorder; `4b46709` chore: ignore logs/ (ephemeral Tier-1 closure output) — `.gitignore` +3. Closed issue #4 with a ratification comment.
- Result: JOURNAL change-record gap (CLAUDE.md §6) closed for the 2026-06-02 Tier-1 install; nightly survivor ACCEPTED and issue #4 closed. Documentation catch-up only — no code or config changed by this entry (the install commits themselves predate it and stand as-is).
- Changes: `JOURNAL.md` (this catch-up entry only). Branch `chore/triage-2026-06-06`, merged `--no-ff` and pushed. Issue #4 closed with ratification comment.
- Abandoned: nothing.
- Next: Nightly conformance routine continues on schedule; the 2026-06-02 install is now journaled, so it should not resurface as a survivor.
...
### 2026-07-07 — chore/adopt-methodology-v1.2.0 (#14, hub #221 n=2)

- Did: Adopted the ecosystem methodology corpus v1.2.0 via the hub deploy tool (.dev-knowledge ADR-91/92; the tool stages, never commits — every commit here is operator-ratified on this branch). Arc: `17478db` filed #14 + genuine CLAUDE.md re-read/restamp v2.3 (executes hub #100; §9 was stale vs the live config) → `712a18e` armed ALL THREE git-hook stages (`default_install_hook_types`; closes the ai-council arming-gap class, hub #275) → `0cab8be` ratified the deploy's 10 staged paths (floor + hash guard + @-include, seb Stop hook + canonical_freshness gate + /override, hub-hooks rev 69558c7→v1.2.0 in place, tier1 plugin installed; corp's own ruff untouched — the remove leg SKIPPED as greenfield per the ADR-96 amendment 2026-07-06) → this commit: `default_stages: [pre-commit]` (stage-less hooks single-fire), corp-owned `.gitignore` negations restoring the tracked-.claude convention, `.methodology.yaml` sanctioning the ruff divergence (review 2026-10-07, pairs with hub #276), CLAUDE.md v2.4 post-deploy reconcile.
- Result: Deploy EXECUTE 5/5 carriers verified ok; remove leg SKIPPED (greenfield); pre-deploy baseline 2588 passed / 5 skipped; ratify commit passed the newly wired gates live (canonical_freshness Passed on the v2.3 restamp; tree clean after — no hook rewrote deployed bytes). Hub record branch deploy/record-corp-monorepo-1.2.0 awaits the hub-side merge (registry n=2). Verification battery (hub #230 self-test, per-stage trip tests, seb fire_test) runs next, pre-merge.
...
### 2026-07-07 — pin functional-architect briefs (T3+T2) to audit library [worktree]

- Did: Pinned two functional-architect briefs from Downloads into `docs/audits/` (exact names, no renames, content untouched) on worktree branch `worktree-ingest-briefs` per the P6 prompt: `2026-07-07_BRIEF_obsidian-operating-model-v2.md` (T3) + `2026-07-07_BRIEF_ontology-north-star.md` (T2). Byte-identical copies (source↔dest sha256 verified for both), one revertable commit per file. The prompt's `ecosystem/*/state.yaml` seed step was MOOT, not skipped: no `ecosystem/` dir exists in the worktree or the primary (nothing to seed from), and corp's `.pre-commit-config.yaml` has NO audit-health hook — only `canonical_freshness` (always_run) gates every commit, and it blocks stale-EDITED canonical docs, not new files in `docs/audits/`. Notably the environmental pre-commit startup hang that forced the sibling golden-url/adopt-map session onto `--no-verify` did NOT recur here — both commits ran the full hook set cleanly (`canonical_freshness Passed`); the wedged python orphans had drained.
- Result: Two commits on `worktree-ingest-briefs`: `7f243a0` (T3 brief, +99) and `63fe813` (T2 brief, +84). Working tree clean. NOT merged — operator runs integration from the primary. merge-base `3b84b3c`; main advanced independently with the SIBLING brief pair (golden-url + adopt-map), so `git merge --no-ff worktree-ingest-briefs` is clean on the audit `.md` files (disjoint adds) but WILL conflict on `JOURNAL.md` — both sibling sessions appended an entry at the same tail. Trivial resolve: keep BOTH entries (stack them).
- Changes: `docs/audits/2026-07-07_BRIEF_obsidian-operating-model-v2.md` (new), `docs/audits/2026-07-07_BRIEF_ontology-north-star.md` (new), this file.
- Abandoned: The `state.yaml` seed — inapplicable to corp (documented, not fabricated). No merge/rebase from the worktree (prompt-forbidden; integration is the operator's from the primary).
- Next: Operator merges `--no-ff worktree-ingest-briefs` from the primary (resolve the JOURNAL.md append conflict by keeping both entries), pushes, tears down the worktree.
