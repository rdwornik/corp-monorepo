...
- **Gotcha:** Never delete archive/cleanup folders without verifying Rob's personal files exist SEPARATELY from underscore (SharePoint) folders [MERGED into 80_Archive cleanup entry below]
  - Trigger: Any bulk delete on OneDrive or MyWork folders
  - Symptom: Rob's files deleted alongside SharePoint copies
  - Fix: See consolidated entry under corp-by-os section
  - verify: manual (OneDrive exclusion zone; verify before any bulk delete)
  - **Last triggered:** 2026-03-25

- **Gotcha:** gemini-2.5-flash fallbacks scattered across repos — all updated to gemini-3-flash-preview as of 2026-03-12
  # [NEEDS VERIFICATION] — check if gemini-3-flash-preview is still current model string
  ...
  - Fix: Always use gemini-3-flash-preview; grep for old model strings before releasing
  - verify: Grep("gemini-2.5-flash|gemini-2.0-flash", path="corp-monorepo/") → 0 matches
  - **Last triggered:** 2026-03-25

- **Gotcha:** CKE v0.4.0 output lacks tags/provenance — don't ingest old output into clean vault
  # [NEEDS VERIFICATION] — check if this gotcha is still relevant
...
- **Gotcha:** source_path breaks when files move — extract only from stable locations
  ...
  - Fix: Only extract from settled locations (60_Source_Library, 50_RFP, 10_Projects)
  - verify: manual (operational rule; extract only from 60_Source_Library, 50_RFP, 10_Projects)
  - **Last triggered:** 2026-03-25

- **Gotcha:** 80_Archive cleanup — verify Rob's files exist outside underscore folders BEFORE delete [MERGED: consolidated from two entries]
  - Trigger: Any bulk delete on OneDrive, MyWork, or 80_Archive folders
...
- (No gotchas recorded in tasks/lessons.md yet)
...
- **Gotcha:** Unknown taxonomy terms are preserved, not dropped — allows organic growth
  ...
  - verify: Grep("taxonomy_review", path="src/corp/schema/") → unknown terms tracked, not dropped
  - **Last triggered:** 2026-03-25
...
- (No gotchas beyond universal az shell=True trap — see global ~/.claude/skills/gotchas/gotchas.md)
