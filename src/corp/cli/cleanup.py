...
def cleanup_scan_command(output: str | None) -> None:
    ...
    # Generate proposals
    output_path = Path(output) if output else mywork_root / CORP_INFRA / "moves.yaml"
...
def apply_moves_command(moves_file: str | None, dry_run: bool) -> None:
    ...
    if moves_file:
        ...
        moves_path = mywork_root / CORP_INFRA / "moves.yaml"
...
def cleanup_cmd(scope: str, execute: bool) -> None:
    ...
    if execute:
        ...
        log_path = cfg.mywork_root / CORP_INFRA / "cleanup_log.jsonl"
        total_deleted = 0
        total_failed = 0
