...
(generator adoption is opt-in per ADR-51 amendment 2026-05-22; corp has
...
<!-- CODEMAP:START -->
```
Package dependency edges (each edge = a higher layer importing a lower one; Tach-enforced).
Layer of each source node in parens; interface > orchestration > core > foundation.

  cli/          (interface)      --> ingest, retrieve, extractor, project, opportunity, rfp
  ingest/       (orchestration)  --> extractor, ops, schema
  extractor/    (core)           --> extraction, schema
  retrieve/     (core)           --> ops, schema
  ops/          (core)           --> schema
  project/      (core)           --> schema
  opportunity/  (core)           --> schema
  rfp/          (core)           --> retrieve

Leaf nodes (imported by the above, import nothing higher): schema/, extraction/ (both foundation).
Module sources: src/corp/{cli,ingest,extractor,retrieve,project,opportunity,rfp,ops,schema,extraction}/.
```
<!-- hand-authored codemap (textual edge list; Mermaid dropped per [#326]) per ADR-51 amendment 2026-05-22; not generator-managed -->
<!-- CODEMAP:END -->
...
and blocked by pre-commit and CI (`tach check`). Layer order (each imports only
downward): `interface → orchestration → core → foundation`. Per-layer module
assignments are enumerated below.
